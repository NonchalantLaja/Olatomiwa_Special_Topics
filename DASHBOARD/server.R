...
shinyServer(function(input, output) {
  ...
  output$S <- renderPrint({
    summary(data2[data2$Company == input$choose_S, ])
  })
  
  output$C <- renderPrint({
    cor(data2[data2$Company == input$choose_C, -c(1,8) ])
  })
  
  output$D <- renderPrint({
    str(data2[data2$Company == input$choose_D, ])
  })
  
  output$download_S <- downloadHandler(
    filename = function(){
      paste("Indi_Sum", "txt", sep = ".")
    },
    
    content = function(file){
      write.txt(summary(data2[data2$Company == input$choose_S, ]), file)
    }
  ...
  output$download_C <- downloadHandler(
    filename = function(){
      paste("Correlation", "txt", sep = ".")
    },
    
    content = function(file){
      write.txt(cor(data2[data2$Company == input$choose_C, -c(1,8) ]), file)
    }
    
  )
  
  output$download_D <- downloadHandler(
    filename = function(){
      paste("description", "txt", sep = ".")
    },
    
    content = function(file){
      write.txt(str(str(data2[data2$Company == input$choose_D, ])), file)
    }
